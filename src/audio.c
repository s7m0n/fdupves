...
int
audio_extract (const char *file, float offset, float length, int ar,
               short **pBuffer, int *pLen)
{
  ...
  if (stream->duration != AV_NOPTS_VALUE)
    {
      total_length = (float)(stream->duration * stream->time_base.num)
  ...
  convert_ctx = swr_alloc_set_opts (
      NULL, AV_CH_LAYOUT_MONO, AV_SAMPLE_FMT_S16, ar,
  ...
  if (convert_ctx)
    {
      int ret = swr_alloc_set_opts2 (
          &convert_ctx, &out_ch_layout, AV_SAMPLE_FMT_S16, ar, &in_ch_layout,
          codec_ctx->sample_fmt, codec_ctx->sample_rate, 0, NULL);
      if (ret < 0)
        {
...
int
audio_extract_to_wav (const char *file, float offset, float length, int ar,
                      const char *out_wav)
{
  short *buf;
  int len, samples;
